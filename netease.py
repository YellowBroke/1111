...
def get_enc(data):
    ...
    secretKey = 'g4PXsCuqYE6icH3R'
    first = encryption(data, presetKey)
    return encryption(first, secretKey)
...
f = open('userIdpa.txt', 'r')
f1 = codecs.open('data.txt', 'a', 'utf-8')
i = 0
for userId in f.readlines():
    userId = userId.strip('\n')
    data = "{\"uid\": \"" + userId + "\", \"type\": 0}"
    params = get_enc(data)
    finalData = {'params': params, 'encSecKey': encSecKey}
    response = requests.post(url=url, data=finalData, headers=header)
    print(i)
    i += 1
    if response.text == "":
        continue
    ...
    if rspJson["code"] != 200:
        continue
    for entry in rspJson["allData"]:
        song = entry["song"]
        try:
            f1.write(userId + ":::" + str(song["id"]) + ":::" + song["name"] + ":::" + str(entry["score"]) + "###")
        except:
            pass
f.close()
f1.close()
